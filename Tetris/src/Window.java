...
public class Window {
	...
	public void actionMouseLeft() {
		if (window.equals("menu")) {
			...
			// go to settings
			if (menuSettings.mouseOverButton()) {
				window = "settings";
			}
			// exit game
			if (menuExit.mouseOverButton()) {
				processing.exit();
			}
		}
		if (window.equals("settings")) {
			...
			if (settingsGeneral.mouseOverButton()) {
				window = "generalSettings";
			}
			// go to write name window
			if (settingsName.mouseOverButton()) {
				window = "writeName";
			}
			// go to the background color window
			if (settingsColor.mouseOverButton()) {
				window = "backgroundColor";
			}
		}
		if (window.equals("game")) {
			// starting the game
			...
			// restart the game
			if ((gameRestart.mouseOverButton() && processing.mouseButton == processing.LEFT)
			...
			if (gameMenu.mouseOverButton() && (processing.mouseButton == processing.LEFT)) {
			...
			// pause the game
			if ((gamePause.mouseOverButton() && processing.mouseButton == processing.LEFT)
		...
		if (window.equals("generalSettings")) {
			// go to settings
			if (generalBack.mouseOverButton()) {
				window = "settings";
			}
			// set window size 1
			if (sizeSize1.mouseOverButton()) {
				settings.setSize(1);
				resize();
				game.refresh();
				background.refresh();
		...
		if (window.equals("scores")) {
			// go to menu
			if (scoresBack.mouseOverButton()) {
				window = "menu";
			}
		}
		processing.mouseButton = 0; // reset the mouse button to avoid artefacts
	}
